...
//     floatingWindow.style.display = visible ? "block" : "none";
//     overlay.style.display = visible ? "block" : "none";
// }

// function showRecoveryMenu() {
//     // 替換 shadow DOM 內容為救援選單
//     shadow.innerHTML = `
//         <style>
//             #main {
//                 all: initial; position: fixed; width: 320px; padding: 15px;
//                 top: calc(50% - 150px); left: calc(50% - 160px); font-family: sans-serif;
//                 background: white; border: 1px solid #ddd; box-shadow: 0 4px 15px rgba(0,0,0,0.2);
//                 z-index: 999999; border-radius: 8px;
//             }
//             .title { text-align: center; margin-bottom: 20px; font-size:20px; font-weight: bold; color: #343a40; }
//             .menu-btn { 
//                 display: block; width: 100%; padding: 10px 15px; margin-bottom: 8px; 
//                 background: #f8f9fa; color: #495057; border: 1px solid #ced4da; 
//                 border-radius: 4px; cursor: pointer; font-size: 16px; text-align: left;
//             }
//             .menu-btn:last-child { margin-top: 15px; background:#dc3545; color:white; }
//         </style>
//         <div id="main">
//             <div class="title">是否需要提升專注? 選取以下功能協助</div>
//             <button id="music-btn" class="menu-btn">🎧 AI 專注音樂 (YouTube)</button>
//             <button id="pomodoro-btn" class="menu-btn">🍅 嵌入式番茄鐘</button>
//             <button id="assist-btn" class="menu-btn">💡 開啟寫作助手</button>
//             <button id="exercise-btn" class="menu-btn">🏃 AI 運動建議</button>
...
//     // --- 動作監聽 ---
//     const menuClose = () => setWindowVisibility(false);

//     shadow.querySelector("#music-btn").addEventListener("click", () => {
//         chrome.runtime.sendMessage({ action: "playFocusMusic" }); 
//         menuClose();
//     });

//     shadow.querySelector("#pomodoro-btn").addEventListener("click", () => {
//     console.log("[Forward] test");
//         alert("🍅 番茄鐘已啟動：開始 25 分鐘工作！（請自行計時）"); 
//         menuClose();
//     });

//     shadow.querySelector("#assist-btn").addEventListener("click", () => {
//         // chrome.runtime.sendMessage({ action: "showAIWindow" }); 
...
//     shadow.querySelector("#exercise-btn").addEventListener("click", () => {
//         alert("💪 站起來伸展 10 次深蹲，休息 1 分鐘！");
//         menuClose();
//     });

//     shadow.querySelector("#close-menu-btn").addEventListener("click", menuClose);
// }
...
`;
    ...
    recoveryContainer.querySelector("#music-btn").addEventListener("click", () => {
        window.open("https://www.youtube.com/results?search_query=focus+music+no+ads+white+noise", "_blank");
        menuClose();
    });
...
chrome.storage.local.get(["keyTimestamps"], (data) => {
    ...
    const lastTyping = timestamps.length ? timestamps.at(-1) : Date.now();
    ...
    console.log("距離上次打字時間:", focusDurationMin, "分鐘");
        const prompt = `現在時間是 ${timeStr}，我已經專注工作${focusDuration} 分鐘。請給我一個簡單運動建議。`;
        ...
        .then(r => r.json())
        .then(data => alert(data.reply || "AI 回覆錯誤"))
